...
MOON_OUTPUT = Path(__file__).resolve().parents[1] / "data" / "moon-cutoff-states.js"
PLANET_CUTOFF_OUTPUT = (
    Path(__file__).resolve().parents[1] / "data" / "planet-cutoff-states.js"
)
...
PLANETS = {
    ...
    "Pluto": {
        "id": "999",
        "step": "80 d",
        "encounters": ["2015-07-14", STOP_DATE],
    },
    "Charon": {
        "id": "901",
        "step": "80 d",
...
def generate_moon_cutoff_states() -> dict[str, object]:
    start = f"{STOP_DATE} 00:00"
    stop = f"{STOP_DATE} 02:00"
    states: dict[str, object] = {}
    for name, target_id in MOONS.items():
        print(f"Fetching {name} cutoff state...")
        points = request_vectors(target_id, start, stop, "1 h")
        states[name] = {
            "source": "NASA/JPL Horizons",
            "targetId": target_id,
            "frame": "Ecliptic J2000",
    ...
    return {
        "generated": date.today().isoformat(),
        "epoch": f"{STOP_DATE}T00:00:00Z",
        "center": "Sun",
        "states": states,
    }


def generate_planet_cutoff_states() -> dict[str, object]:
    start = f"{STOP_DATE} 00:00"
    stop = f"{STOP_DATE} 02:00"
    states: dict[str, object] = {}
    for name, planet in PLANETS.items():
        print(f"Fetching {name} cutoff state...")
        points = request_vectors(planet["id"], start, stop, "1 h")
        states[name] = {
            "source": "NASA/JPL Horizons",
            "targetId": planet["id"],
            "frame": "Ecliptic J2000",
...
def write_planet_cutoff_states() -> None:
    payload = generate_planet_cutoff_states()
    encoded = json.dumps(payload, separators=(",", ":"), ensure_ascii=True)
    PLANET_CUTOFF_OUTPUT.write_text(
        "// Generated by tools/fetch-trajectories.py; do not edit manually.\n"
        f"window.PLANET_CUTOFF_STATES={encoded};\n",
        encoding="utf-8",
    )
    print(
        f"Wrote {PLANET_CUTOFF_OUTPUT} "
        f"({PLANET_CUTOFF_OUTPUT.stat().st_size:,} bytes)"
    )


def write_moon_cutoff_states() -> None:
    payload = generate_moon_cutoff_states()
    encoded = json.dumps(payload, separators=(",", ":"), ensure_ascii=True)
    MOON_OUTPUT.write_text(
        "// Generated by tools/fetch-trajectories.py; do not edit manually.\n"
        f"window.MOON_CUTOFF_STATES={encoded};\n",
        encoding="utf-8",
    )
    print(f"Wrote {MOON_OUTPUT} ({MOON_OUTPUT.stat().st_size:,} bytes)")
...
def main() -> None:
    ...
    parser.add_argument(
        "--moons-only",
        action="store_true",
        help="Generate only the compact moon cutoff-state file.",
    )
    parser.add_argument(
        "--planet-cutoff-only",
        action="store_true",
        help="Generate only the compact planet cutoff-state file.",
    )
    args = parser.parse_args()
    OUTPUT.parent.mkdir(parents=True, exist_ok=True)
    if args.moons_only:
        write_moon_cutoff_states()
        return
    if args.planet_cutoff_only:
        write_planet_cutoff_states()
        return
